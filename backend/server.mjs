...
import execRoutes from "./routes/execs.js";
...
app.use("/api/submit-contact-form", contactRoutes);
...
// Start server
connectToDatabase(process.env.mongoDBUrI).then(() => {
  app.listen(PORT, () => {
...
import Execs from "./models/ExecSchema.mjs";
...
async function fixAllIndexes() {
  try {
    ...
    await mongoose.connection
      .collection("executives")
      .createIndex({ studentId: 1, session: 1 }, { unique: true });
    console.log("Created studentId+session compound index");

    await mongoose.connection
      .collection("executives")
      .createIndex({ position: 1, session: 1 }, { unique: true });
    console.log("Created position+session compound index");

    await mongoose.connection
      .collection("executives")
      .createIndex({ email: 1, session: 1 }, { unique: true });
    ...
    try {
      ...
      if (existingExecWithEmail) {
        return res.status(400).json({
          error: true,
          message:
            "An executive with this email already exists in this session",
        });
      }
      ...
      if (existingExecWithStudentId) {
        return res.status(400).json({
          error: true,
          message:
            "An executive with this student ID already exists in this session",
        });
      }
